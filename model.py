...
def prediction(stock, n):
    ...
    #grid search to get best parameters for ML model.
    gsc = GridSearchCV(
    ...
    #list of index of days that needs to run regression model on  
    output_days = list()
    for i in range(1, n):
        output_days.append([i + x_test[-1][0]])
        
    #list of dates that needs to predict, x variable
    dates = []
    current = date.today()
    for i in range(n):
    ...
    fig.update_layout(
        title = "Prediction of next " + str(n - 1) + " days",
        xaxis_title = "Date",
